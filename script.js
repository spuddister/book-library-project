...
function formValidation() {
  ...
  if (!authorInput.checkValidity()) {
    authorInput.classList.add("is-danger");
  } else {
    authorInput.classList.remove("is-danger");
  }

  if (!pagesInput.checkValidity()) {
    pagesInput.classList.add("is-danger");
  } else {
    pagesInput.classList.remove("is-danger");
  }
...
function submitNewBook() {
  ...
  authorInput.value == ""
    ? (newBookObject.author = "Author: Unknown")
    : (newBookObject.author = authorInput.value);
  pagesInput.value == ""
    ? (newBookObject.pages = "N/A")
    : (newBookObject.pages = pagesInput.value);
  readRadioBtn.checked
    ? (newBookObject.read = true)
    : (newBookObject.read = false);
...
function toggleRead(index) {
  myLibrary[index.getAttribute("bookindex")].read =
    !myLibrary[index.getAttribute("bookindex")].read;
  repopLibrary();
}
